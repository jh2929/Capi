...
import com.capi.music.App
import com.capi.music.constants.GitHubReleasesEtagKey
import com.capi.music.constants.GitHubReleasesFingerprintKey
import com.capi.music.constants.GitHubReleasesJsonKey
import com.capi.music.constants.GitHubReleasesLastCheckedAtKey
import com.capi.music.constants.UpdateChannel
import com.capi.music.constants.UpdateChannelKey
...
private const val APK_ASSET_NAME = "capi-universal-debug.apk"
...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/jh2929/Capi-TV/releases?per_page=$perPage") {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "Capi")
    ...
    private suspend fun fetchNightlyJson(): Result<NightlyInfo> = runCatching {
        ...
        NightlyInfo(
            ...
            apkUrl = fixedApkUrl.ifEmpty {
                "https://github.com/jh2929/Capi-TV/releases/latest/download/capi-universal-debug.apk"
            },
    ...
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/jh2929/Capi-TV/releases/download/$tagName/$APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/jh2929/Capi-TV/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "Capi")
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/jh2929/Capi-TV/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", "Capi")
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/jh2929/Capi-TV/releases/latest/download/$APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
