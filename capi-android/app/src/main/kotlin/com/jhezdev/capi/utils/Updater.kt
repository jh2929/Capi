...
import com.jhezdev.capi.App
import com.jhezdev.capi.constants.GitHubReleasesEtagKey
import com.jhezdev.capi.constants.GitHubReleasesFingerprintKey
import com.jhezdev.capi.constants.GitHubReleasesJsonKey
import com.jhezdev.capi.constants.GitHubReleasesLastCheckedAtKey
import com.jhezdev.capi.constants.UpdateChannel
import com.jhezdev.capi.constants.UpdateChannelKey
...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/Jhezdev/Capi/releases?per_page=$perPage") {
                headers {
    ...
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/Jhezdev/Capi/releases/download/$tagName/$APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/Jhezdev/Capi/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "Capi")
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/Jhezdev/Capi/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", "Capi")
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/Jhezdev/Capi/releases/latest/download/$APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
