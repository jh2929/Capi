...
import { 
  ...
  Moon, Timer, BarChart3, Languages, RotateCcw, Trophy, Users, Repeat, Repeat1, Lock, Unlock, Upload, Pencil, HardDrive,
  FileText
} from "lucide-react";
import "./App.css";
...
import { diagnostics, DiagnosticModal } from "./diagnostics";
import type { DiagnosticReport } from "./diagnostics";
import { parseAndDetectLyrics } from "./lyricsParser";
import { castManager, CastSession } from "./castManager";

const GlassesIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="6" cy="14" r="4" />
    <circle cx="18" cy="14" r="4" />
    <path d="M14 14a2 2 0 0 0-4 0" />
    <path d="M2.5 12.5 4 4" />
    <path d="M21.5 12.5 20 4" />
  </svg>
);

const CastIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M2 16.1A5 5 0 0 1 5.9 20M2 12.05A9 9 0 0 1 9.95 20M2 8V6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-6" />
    <line x1="2" y1="20" x2="2.01" y2="20" strokeWidth="3" strokeLinecap="round" />
  </svg>
);
...
const getBannerUrl = (url: string) => {
  ...
  if (url.includes("googleusercontent.com") || url.includes("ggpht.com")) {
    ...
    if (match) {
      ...
      const scale = 1920 / w;
      return url.replace(/=w\d+-h\d+/, `=w1920-h${Math.round(h * scale)}`);
    }
    const pathMatch = url.match(/\/w(\d+)-h(\d+)/);
    if (pathMatch) {
...
function App() {
  ...
  const activeQueueRef = useRef<Track[]>([]);
  ...
  const getPlaylistCover = (playlist: Playlist): string => {
    const stored = JSON.parse(localStorage.getItem("capi_playlist_covers") || "{}");
    return stored[playlist.id] || playlist.tracks?.[0]?.thumbnail || "";
  };
  const [libTab, setLibTab] = useState<"home" | "downloads" | "history" | "local">("home");
  ...
  useEffect(() => {
    ...
    root.classList.remove("theme-ultra-dark", "theme-light-mode", "theme-midnight-blue", "theme-forest-green", "theme-sunset-orange", "theme-ocean-blue", "theme-cherry-blossom", "theme-nordic", "theme-dracula");
  ...
  const [isLyricsSynced, setIsLyricsSynced] = useState<boolean>(false);
  const [lyricsDisplayMode, setLyricsDisplayMode] = useState<"synced" | "plain">("synced");
  const [lyricsLoadingProvider, setLyricsLoadingProvider] = useState<string | null>(null);
  const lyricsReqIdRef = useRef<number>(0);
  const lyricsCacheRef = useRef<Map<string, { lyrics: string; provider: string; isSynced: boolean; lines: LyricLine[] }>>(new Map());
  ...
  const [castSession, setCastSession] = useState<CastSession>(castManager.getSession());
  const [manualCastIp, setManualCastIp] = useState<string>("");
  ...
  const accentBtnRef = useRef<HTMLButtonElement | null>(null);
  ...
  // Subscribe to Cast Session
  useEffect(() => {
    const unsub = castManager.subscribe(sess => {
      setCastSession(sess);
    });
    return unsub;
  }, []);

  // Initialize the diagnostics/observability layer and gather system info
  useEffect(() => {
    diagnostics.init();
    invoke<string>("obtener_info_sistema")
      .then(raw => {
        try {
          diagnostics.setBackendInfo(JSON.parse(raw));
        } catch (e) {
          console.warn("[DIAG] No se pudo parsear la info del sistema:", e);
        }
      })
      .catch(err => console.warn("[DIAG] No se pudo obtener la info del sistema:", err));
  }, []);
  ...
  const [searchHistory, setSearchHistory] = useState<string[]>(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0 && typeof parsed[0] === "object") {
        return parsed.map((h: any) => h.query || "");
      }
      return Array.isArray(parsed) ? parsed : [];
    } catch { return []; }
  });
  const [searchThumbnails, setSearchThumbnails] = useState<Record<string, string>>(() => {
    try { return JSON.parse(localStorage.getItem("capi_search_thumbnails") || "{}"); } catch { return {}; }
  });
  ...
  // ═══════════════════════════════════════════════════════════════
  const [pillPlayer, setPillPlayer] = useState<boolean>(() =>
    localStorage.getItem("capi_pill_player") === "true"
  );
  const [pillRadius, setPillRadius] = useState<number>(() => {
    const saved = localStorage.getItem("capi_pill_radius");
    return saved ? parseInt(saved, 10) : 64;
  });
  const [showPillRadius, setShowPillRadius] = useState(false);
  ...
  // ─── MPRIS Control listener (from proxy API) ───────────────────────
  useEffect(() => {
    const unlisten = listen<any>("mpris-command", (event) => {
      ...
      switch (payload.action) {
        ...
        case "seek":
          ...
          break;
        case "volume":
          const clamped = Math.max(0, Math.min(1, payload.level));
          setVolume(clamped);
  ...
  // ─── Daemon lifecycle listener ──────────────────────────────────
  useEffect(() => {
    const unlistenRestart = listen<{ timestamp: string; reason: string }>("daemon-restarted", (event) => {
      console.log(`[DAEMON] Restarted (${event.payload.reason}) at ${event.payload.timestamp}`);
      diagnostics.onDaemonRestarted(event.payload.reason, event.payload.timestamp);
      streamCacheRef.current = {};
      showToast("Motor de streaming reiniciado");
    });
    const unlistenReady = listen("daemon-ready", () => {
      diagnostics.onDaemonReady();
      showToast("Motor de streaming listo");
    });
    return () => {
  ...
  // ─── Emit media state to Rust backend every 3s ────────────────────
  const emitMediaState = useCallback(() => {
    if (!currentTrack) return;
    invoke("media_update_state", {
      newState: {
  ...
  }, [currentTrack, isPlaying, currentTime, duration, volume, isShuffle, repeatMode, streamUrl]);

  useEffect(() => {
    emitMediaState();
    if (!isPlaying || !currentTrack) return;
    const interval = setInterval(emitMediaState, 3000);
    return () => clearInterval(interval);
  }, [emitMediaState, isPlaying, currentTrack]);

  // Track scroll position for immersive header/sidebar borders
  useEffect(() => {
    ...
    const handler = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          if (activeTab === "artist") {
            setArtistScrolled(el.scrollTop > 5);
            const banner = document.getElementById("artist-banner-bg");
            if (banner) {
              banner.style.transform = `translateY(-${el.scrollTop}px)`;
            }
          }
          ticking = false;
        });
  ...
  useEffect(() => { localStorage.setItem("capi_search_thumbnails", JSON.stringify(searchThumbnails)); }, [searchThumbnails]);
  useEffect(() => { localStorage.setItem("capi_show_sidebar_settings", String(showSidebarSettings)); }, [showSidebarSettings]);

  // Backfill searchThumbnails from current tracks when search completes
  useEffect(() => {
    if (tracks.length > 0 && query.trim()) {
      setSearchThumbnails(prev => {
        if (prev[query.trim()]) return prev;
        return { ...prev, [query.trim()]: tracks[0].thumbnail };
      });
    }
  }, [tracks, query]);
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (isPlayerExpanded) {
        if (e.key === "ArrowRight") {
          ...
          const newTime = dur * pct;
          if (audioRef.current && dur > 0) {
            audioRef.current.currentTime = newTime;
            setCurrentTime(newTime);
          }
          setToastData({ kind: "text", message: `${Math.round(pct * 100)}%` });
          dismissArrow();
        } else if (e.key.startsWith("Numpad") && e.key.length === 7) {
          e.preventDefault();
          const pct = parseInt(e.key[6]) / 10;
          const dur = audioRef.current?.duration ?? 0;
          const newTime = dur * pct;
          if (audioRef.current && dur > 0) {
            audioRef.current.currentTime = newTime;
  ...
  const syncLyricsScroll = useCallback((force = false) => {
    if (lyricsDisplayMode === "plain" || !isLyricsSynced) return;
    ...
    const index = curLyrics.findIndex((line, i) => {
      ...
      return line.time >= 0 && curTime >= line.time && (!nextLine || nextLine.time < 0 || curTime < nextLine.time);
    });
    if (index !== -1) {
      setCurrentLyricIndex(index);
      if ((lyricsScrollLocked || force) && (force || index !== lastScrolledIndexRef.current)) {
        ...
        if (activeEl && lyricsContainerRef.current) {
          ...
          const targetTop = activeEl.offsetTop - lyricsContainerRef.current.clientHeight / 2 + activeEl.clientHeight / 2;
  ...
  }, [lyricsScrollLocked, lyricsDisplayMode, isLyricsSynced]);
  ...
  // Fetch lyrics and reset slider when track changes
  useEffect(() => {
    if (currentTrack) {
      setLyricsProvider(null);
      setShowRemaining(false);
      fetchLyrics(currentTrack);
      fetchRelatedTracks(currentTrack.artist);
    }
    
    // Reset playback bar DOM elements on track change
    const tMin = document.getElementById("player-time-min");
    const tExp = document.getElementById("player-time-exp");
    const sMin = document.getElementById("player-slider-min") as HTMLInputElement | null;
    const sExp = document.getElementById("player-slider-exp") as HTMLInputElement | null;
    
    if (tMin) tMin.textContent = "00:00";
    if (tExp) tExp.textContent = "00:00";
    if (sMin) sMin.value = "0";
    if (sExp) sExp.value = "0";
  }, [currentTrack?.id]);

  // Close lyrics menu on click outside
  useEffect(() => {
    if (!lyricsMenuOpen) return;
    const handleClick = (e: MouseEvent) => {
      if (lyricsMenuRef.current && !lyricsMenuRef.current.contains(e.target as Node)) {
  ...
  useEffect(() => {
    ...
    if (card && firstTrack && firstTrack.artistId) {
      ...
      invoke<string>("obtener_artista", { id: firstTrack.artistId }).then(res => {
        ...
        setArtistCardData(prev => prev ? {
          ...prev,
          name: data.name || prev.name,
          thumbnail: getHighQualityThumbnail(data.thumbnail || prev.thumbnail),
  ...
  const fetchNewReleases = async () => {
    try {
      const response = await invoke<string>("obtener_nuevos_lanzamientos");
      const data = JSON.parse(response);
      const items = Array.isArray(data) ? data : [];
      if (items.length > 0) {
        setNewReleases(items.map(convertYTItemToTrack));
      } else {
  ...
  const loadAlbumProfile = async (id: string, title: string, thumbnail: string, type: string, artistName?: string) => {
    ...
    try {
      ...
      setErrorModal({ message: "Error al cargar las canciones del álbum." });
    }
  };
  ...
  const fetchLyrics = async (track: Track, providerOverride?: string, forceRefresh: boolean = false) => {
    const reqId = ++lyricsReqIdRef.current;
    const providerKey = providerOverride || "auto";
    const cacheKey = `${track.id}::${providerKey}`;

    if (!forceRefresh && lyricsCacheRef.current.has(cacheKey)) {
      const cached = lyricsCacheRef.current.get(cacheKey)!;
      setLyricsProvider(cached.provider);
      setLyricsText(cached.lyrics);
      setIsLyricsSynced(cached.isSynced);
      setParsedLyrics(cached.lines);
      setCurrentLyricIndex(-1);
      setLyricsLoadingProvider(null);
      return;
    }

    setLyricsLoadingProvider(providerOverride || "Automático");
    setLyricsText(providerOverride ? `Buscando letras en ${providerOverride}...` : "Cargando letras...");
    setParsedLyrics([]);
    setCurrentLyricIndex(-1);
    ...
    try {
      ...
      if (parsed && parsed.lyrics) {
        const activeProv = parsed.provider || providerOverride || "Auto";
        setLyricsProvider(activeProv);
        const rawText = parsed.lyrics;
        setLyricsText(rawText);

        const detected = parseAndDetectLyrics(rawText, track.duration);
        setIsLyricsSynced(detected.isSynced);
        setParsedLyrics(detected.lines);

        // Cache result for quick switching
        lyricsCacheRef.current.set(cacheKey, {
          lyrics: rawText,
          provider: activeProv,
          isSynced: detected.isSynced,
          lines: detected.lines
        });
      } else {
        setLyricsText(providerOverride ? `No se encontraron letras en ${providerOverride}.` : "Letras no disponibles.");
        setParsedLyrics([]);
        setIsLyricsSynced(false);
      }
    } catch (e) {
      ...
      setParsedLyrics([]);
      setIsLyricsSynced(false);
      console.error("[Lyrics] Error fetching lyrics:", e);
    } finally {
  ...
  // Extract colors whenever track changes or player expands
  useEffect(() => {
    if (currentTrack) {
  ...
  const wrapText = (ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number, align: CanvasTextAlign = "center") => {
  ...
  const translateLyrics = async (targetLang: string = translationLang || "es") => {
    if (!targetLang || (parsedLyrics.length === 0 && !lyricsText)) return;
    setTranslating(true);
    setLyricsMenuOpen(false);
    ...
    try {
      ...
      if (parsedLyrics.length > 0) {
        ...
        setTranslatedLyrics(translatedLines.map(t => ({ time: -1, text: t })));
      }
  ...
  const playTrack = async (track: Track, queueList: Track[] = []) => {
    ...
    // Señal al sistema de diagnóstico: nueva solicitud de reproducción
    diagnostics.onPlaybackRequested(track);
    ...
    try {
      ...
      if (isPlaylistId) {
        ...
        setLoading(true);
        setBuffering(true);
        diagnostics.onPlaylistResolveStarted();
        try {
          ...
          if (songs.length > 0) {
            ...
            setLoading(false);
            setBuffering(false);
            diagnostics.onPlaylistResolved(songs.length);
          ...
          diagnostics.onPlaylistResolveFailed(err);
          diagnostics.onPlaybackFailed(track, err);
          setDiagnosticReport(diagnostics.buildReport());
          setLoading(false);
          setBuffering(false);
          return;
        }
      ...
      diagnostics.onMetadataReady(track);
      ...
      let stream: string;
      let streamSource: "local" | "download" | "cache" | "backend" = "backend";
      const resolveStart = performance.now();
      if (track.id.startsWith("local://")) {
        streamSource = "local";
        diagnostics.onStreamResolveStarted(streamSource);
        ...
        while (true) {
          try {
            resultJson = await invoke<string>("obtener_stream", { id: track.id });
          } catch (e) {
            if (retries < 2) {
              retries++;
              console.warn(`[PLAY] Stream retry ${retries} for ${track.title}`);
              diagnostics.onStreamResolveRetry(retries);
              await new Promise(r => setTimeout(r, 1500));
              continue;
            }
            diagnostics.onStreamResolveFailed(e);
            throw e;
          }
          break;
        }
        ...
        if (parsedError) {
          diagnostics.onStreamResolveFailed(parsedError);
          throw parsedError;
        }
      ...
      // Validar la URL final del stream y registrarla en el diagnóstico
      const validation = diagnostics.validateStreamUrl(stream);
      diagnostics.onStreamResolved(stream, streamSource, performance.now() - resolveStart, validation);
      if (!validation.ok) {
      ...
      if (audioRef.current) {
        ...
        audioRef.current.play().then(() => {
          ...
          setBuffering(false);
          setLoading(false);
          diagnostics.onPlaybackStarted(track);
          // Transmit track stream to connected Cast device if active
          castManager.transmitTrack({ id: track.id, title: track.title, artist: track.artist, url: stream, thumbnail: track.thumbnail });
      ...
      diagnostics.onPlaybackFailed(track, error);
      const report = diagnostics.buildReport();
      console.log("[DIAG] modal de diagnóstico:", report.summary.name);
  ...
  const downloadTrack = async (track: Track) => {
    ...
    try {
      ...
      if (activeDownloadsRef.current[track.id]) {
        ...
        setErrorModal({ message: "Error al descargar." });
      }
    } finally {
  ...
  const deleteLocalTrack = async (trackId: string) => {
    ...
    try {
      ...
      setErrorModal({ message: "No se pudo borrar el archivo: " + e });
    }
  };
  ...
  const performSearchWithQuery = async (searchQuery: string) => {
    ...
    try {
      ...
      if (parsedTracks.length > 0) {
        setSearchThumbnails(prev => ({ ...prev, [searchQuery.trim()]: parsedTracks[0].thumbnail }));
      }
    } catch (error: any) {
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current) {
      ...
      // Always keep the ref in sync so syncLyricsScroll gets exact time
      currentTimeRef.current = cur;
      // Throttled state update to avoid rendering the 7000+ line App component 25 times per second
      if (Math.abs(lastTimeUpdateRef.current - cur) > 1.2) {
        lastTimeUpdateRef.current = cur;
        setCurrentTime(cur);
      }
      // High-frequency lyrics sync (no state change unless line changes)
      const curLyrics = parsedLyricsRef.current;
      if (curLyrics.length > 0) {
        const idx = curLyrics.findIndex((line, i) => {
          const nextLine = curLyrics[i + 1];
          return cur >= line.time && (!nextLine || cur < nextLine.time);
        });
        if (idx !== -1) {
          setCurrentLyricIndex(idx);
          if (lyricsScrollLocked && lyricsContainerRef.current) {
            const activeEl = document.getElementById(`lyric-line-${idx}`);
            if (activeEl) {
              ignoreNextScrollRef.current = true;
              if (autoScrollTimeoutRef.current) window.clearTimeout(autoScrollTimeoutRef.current);
              autoScrollTimeoutRef.current = window.setTimeout(() => { ignoreNextScrollRef.current = false; }, 800);
              lyricsContainerRef.current.scrollTo({
      ...
      // Instant DOM updates for smooth UI
      const tMin = document.getElementById("player-time-min");
      const tExp = document.getElementById("player-time-exp");
      const sMin = document.getElementById("player-slider-min") as HTMLInputElement | null;
      const sExp = document.getElementById("player-slider-exp") as HTMLInputElement | null;
      
      const displayVal = showRemainingRef.current ? duration - cur : cur;
      const formatted = showRemainingRef.current ? `-${formatTime(displayVal)}` : formatTime(displayVal);
      if (tMin) tMin.textContent = formatted;
      if (tExp) tExp.textContent = formatted;
      if (sMin) sMin.value = cur.toString();
      if (sExp) sExp.value = cur.toString();
    }
  };
  ...
  const handleSeek = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Instant time update during dragging
    const tMin = document.getElementById("player-time-min");
    const tExp = document.getElementById("player-time-exp");
    const displayVal = showRemainingRef.current ? duration - val : val;
    const formatted = showRemainingRef.current ? `-${formatTime(displayVal)}` : formatTime(displayVal);
    if (tMin) tMin.textContent = formatted;
    if (tExp) tExp.textContent = formatted;
  };
    ...
    <div className="flex h-screen w-screen bg-bg-dark text-text-primary overflow-hidden font-sans">
      <audio
        ...
        onLoadedMetadata={() => {
          ...
          diagnostics.onPlayerLoadedMetadata(audioRef.current?.duration ?? 0);
        }}
        onPlay={() => setIsPlaying(true)}
        onPause={() => {
          setIsPlaying(false);
          setBuffering(false);
          diagnostics.onPlayerPaused();
          if (listenedAccumRef.current.trackId && listenedAccumRef.current.ms > 0) {
            addListenedTime(listenedAccumRef.current.trackId, listenedAccumRef.current.ms);
            listenedAccumRef.current = { trackId: "", ms: 0 };
          }
        ...
        onWaiting={() => {
          setBuffering(true);
          diagnostics.onPlayerWaiting();
        }}
        onPlaying={() => {
          setBuffering(false);
          diagnostics.onPlayerPlaying();
        }}
        onSeeking={() => setBuffering(true)}
        onSeeked={() => setBuffering(false)}
        onCanPlay={() => {
          setBuffering(false);
          diagnostics.onPlayerCanPlay();
        }}
        onStalled={() => {
          setBuffering(false);
          diagnostics.onPlayerStalled();
        }}
        ...
        onError={(e) => {
          ...
          diagnostics.onPlayerError({
            code: target.error?.code ?? -1,
            message: target.error?.message ?? "",
            networkState: target.networkState,
            readyState: target.readyState,
          });
          // Si la reproducción aún estaba en curso y el audio falla, cerrar la sesión
          // y mostrar el diagnóstico (el evento 'Playing' nunca llegará).
          if (diagnostics.getSessionStatus() === "running") {
            const err = new Error(
              `MediaError ${target.error?.code ?? -1}${target.error?.message ? `: ${target.error.message}` : ""}`
            );
            if (currentTrack) {
              diagnostics.onPlaybackFailed(currentTrack, err);
              setDiagnosticReport(diagnostics.buildReport());
            }
          }
        }}
        onEnded={() => {
          diagnostics.onPlayerEnded();
      ...
      {activeTab === "artist" && artistData?.thumbnail && (
        <div 
          id="artist-banner-bg"
          className="fixed top-0 left-0 z-0 w-screen h-[60vh] pointer-events-none bg-bg-dark overflow-hidden"
        >
          <div className="absolute inset-0 bg-bg-dark">
            <img
              src={getBannerUrl(artistData.thumbnail)}
              alt=""
              className="w-full h-full object-cover absolute left-0 top-0 object-[center_40%]"
              draggable={false}
              onError={(e) => {
                e.currentTarget.src = artistData.thumbnail;
              }}
            />
          </div>
          {/* Side fade to dark background on the left */}
          <div className="absolute inset-0 bg-gradient-to-r from-bg-dark via-bg-dark/50 to-transparent pointer-events-none md:block hidden" />
          
          {/* Bottom fade transition to content - smooth, diffuse, no straight line */}
          <div 
            className="absolute bottom-0 left-0 right-0 h-48 pointer-events-none"
            style={{
              background: "linear-gradient(to top, var(--bg-dark) 0%, rgba(12, 10, 15, 0.9) 30%, rgba(12, 10, 15, 0.4) 65%, rgba(12, 10, 15, 0) 100%)"
            }}
          />
          
          {/* Top fade for header readability */}
          <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/5 to-transparent pointer-events-none" />

          <div className={`absolute top-0 left-0 right-0 p-8 pb-0 transition-all duration-300 ${
            sidebarCollapsed ? "pl-[104px]" : "pl-[288px]"
          }`}>
            <button
              onClick={goBack}
              disabled={navIndex === 0}
              className="p-1 disabled:opacity-30 transition text-white hover:text-white/80 pointer-events-auto"
              title="Atrás"
            >
          ...
          <div className={`absolute bottom-0 left-0 right-0 p-8 pb-12 transition-all duration-300 ${
            sidebarCollapsed ? "pl-[104px]" : "pl-[288px]"
          }`}>
            <h2 className="text-4xl font-extrabold tracking-tight text-white drop-shadow-lg">{artistData.name}</h2>
            {artistData.description && (
              <p className="text-sm text-text-secondary mt-1 max-w-2xl line-clamp-2 drop-shadow-sm">{artistData.description}</p>
            )}
          </div>
        </div>
      )}
      ...
      {/* Sidebar Navigation */}
      <aside
        style={isPlayerExpanded && extractedColors ? { background: `linear-gradient(180deg, ${extractedColors.primary}55 0%, ${extractedColors.tertiary}20 100%)`, borderRightColor: `${extractedColors.primary}45` } : {}}
        className={`${sidebarCollapsed ? "w-[72px]" : "w-64"} ${activeTab === "artist" && (!isPlayerExpanded || !extractedColors) ? (artistScrolled ? "bg-[#030303]" : "bg-transparent") : !isPlayerExpanded || !extractedColors ? "bg-bg-dark" : ""} flex flex-col justify-between transition-all duration-300 z-20 ${
        scrolled ? "border-r border-white/5" : "border-r border-transparent"
      }`}>
        <div className="p-4">
        ...
        <div className={`p-4 flex flex-col gap-2 ${sidebarCollapsed ? "items-center" : ""}`}>
          {showSidebarSettings && (
            <button
      ...
      {/* Main Content */}
      <main className={`flex-1 flex flex-col min-w-0 z-10 relative ${activeTab === "artist" ? "bg-transparent" : "bg-gradient-to-b from-surface-dark/40 to-bg-dark"}`}>
        <header className={`h-20 flex items-center px-8 justify-between gap-4 transition-all duration-300 ${
          activeTab === "artist"
            ? (artistScrolled ? "bg-[#030303]" : "bg-transparent")
            : ""
        } ${scrolled ? "border-b border-white/5" : "border-b border-transparent"} relative z-30`}>
                      ...
                      {query.trim() === "" && (
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            setSearchThumbnails(prev => { const rest = { ...prev }; delete rest[item]; return rest; });
                          }}
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        triggerConfirm(
                          ...
                          () => { setSearchHistory([]); setSearchThumbnails({}); }
                        );
                      }}
        ...
        <section ref={scrollableSectionRef} className={`flex-1 overflow-y-auto p-8 pb-32 ${activeTab === "artist" ? "pt-[60vh]" : ""}`} style={{ willChange: "transform", overscrollBehavior: "contain" }}>
              ...
              {homeSections.length > 0 ? (
                <>
                  ...
                  {carouselTracks.length > 0 && (
                    <div className="relative w-[calc(100%+4rem)] h-64 md:h-80 -mx-8">
                      {[-2, -1, 0, 1, 2].map((offset) => {
                        const idx = (carouselIndex + offset + carouselTracks.length) % carouselTracks.length;
                        const track = carouselTracks[idx];
                        const isActive = offset === 0;
                        const dist = Math.abs(offset);
                        const dir = offset < 0 ? -1 : 1;
                        const step = 9;
                        const zMap: Record<number, number> = { 0: 20, 1: 12, 2: 11 };

                        return (
                          <div
                            key={`${track.id}-${offset}`}
                            onClick={() => {
                              if (!isActive) setCarouselIndex(idx);
                              else playTrack(track);
                            }}
                            className={`absolute top-0 h-full rounded-3xl overflow-hidden shadow-xl cursor-pointer transition-[transform,filter] duration-500 ${
                              isActive ? "z-20" : ""
                            }`}
                            style={{
                              width: "68%",
                              ...
                              transform: isActive
                                ? "translateX(0)"
                                : `translateX(${dir * dist * step}%)`,
                              zIndex: zMap[dist] || 10,
                              filter: isActive ? "none" : "grayscale(0.5)",
                              willChange: "transform",
                            }}
                          >
                            <SafeImage src={track.thumbnail} className="absolute inset-0 w-full h-full object-cover" alt="" />
                            <div className="absolute inset-0 scale-105 blur-lg opacity-50">
                              <SafeImage src={track.thumbnail} className="w-full h-full object-cover" alt="" />
                            </div>
                            <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" />
                            {isActive && (
                              <div className="absolute inset-0 flex items-center justify-between px-8 md:px-16 z-10 gap-4">
                                <div className="text-left max-w-md">
                                  <span className="text-xs uppercase font-bold tracking-widest text-brand-primary">Destacado</span>
                                  <h2 className="text-xl md:text-3xl font-extrabold text-white tracking-tight mt-1 line-clamp-2">
                                    {track.title}
                                  </h2>
                                ...
                                <div className="w-24 h-24 md:w-36 md:h-36 rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex-shrink-0">
                                  <SafeImage src={track.thumbnail} className="w-full h-full object-cover" alt={track.title} />
                                </div>
                              </div>
                            )}
                          </div>
                          ...
                          {playlists.map((playlist) => {
                            ...
                            return (
                              <div
                                ...
                                className="min-w-[200px] w-[200px] flex flex-col group relative cursor-pointer"
                              >
                                <div className="relative aspect-square rounded-xl overflow-hidden mb-2 bg-black/20 shadow-md">
                                  <SafeImage src={coverUrl || "https://images.unsplash.com/photo-1614680376593-902f74fa0d41?q=80&w=300"} alt={playlist.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                                  ...
                                  <button
                                    ...
                                    className="absolute bottom-2 right-2 z-10 w-10 h-10 rounded-full bg-brand-primary flex items-center justify-center text-bg-dark shadow-lg opacity-0 group-hover:opacity-100 transition translate-y-1 group-hover:translate-y-0 hover:scale-105"
                                  >
                                    <Play className="w-4 h-4 fill-current" />
                                  </button>
                                </div>
                                <h4 className="font-semibold text-sm truncate text-text-primary">{playlist.name}</h4>
                      ...
                      <div className="relative group/section">
                        ...
                        <div className="scroll-edge-fade">
                          <div
                            ...
                            className="flex gap-6 pb-2 overflow-x-auto overflow-y-hidden items-center"
                            style={{ scrollbarWidth: "none", msOverflowStyle: "none", touchAction: "pan-x" }}
                          >
                            ...
                            >
                              <div className="relative aspect-square rounded-xl overflow-hidden mb-2 bg-black/20 shadow-md">
                                ...
                                <button 
                                  ...
                                  className="absolute top-2 right-2 z-10 p-1.5 rounded-full bg-black/60 text-white opacity-0 group-hover:opacity-100 transition hover:bg-brand-primary"
                                >
                                ...
                                <div className="absolute bottom-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition translate-y-1 group-hover:translate-y-0">
                                  <button 
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      if (currentTrack?.id === track.id) {
                                    ...
                                    className="w-10 h-10 rounded-full bg-brand-primary flex items-center justify-center text-bg-dark shadow-lg hover:scale-105 transition"
                                  >
                              ...
                              <h4 className="font-semibold text-sm truncate text-text-primary">{track.title}</h4>
                              <div className="flex items-center gap-1.5 mt-0.5 min-w-0">
                                <p 
                                  onClick={(e) => { e.stopPropagation(); item.artists?.[0]?.id && loadArtistProfile(item.artists[0].id); }} 
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {EXPLORE_GENRES.map((genre, idx) => (
                  <div
                    ...
                    style={{
                      background: `linear-gradient(160deg, ${genre.color}22, ${genre.color}cc)`,
                    }}
                    className="relative p-5 rounded-2xl h-36 flex items-end cursor-pointer hover:scale-[1.03] hover:shadow-xl active:scale-95 transition-all duration-200 overflow-hidden group border border-white/5"
                  >
                    <div
                      className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      style={{
                        background: `linear-gradient(160deg, ${genre.color}44, ${genre.color})`,
                      }}
                    />
                    <div
                      className="absolute -top-4 -right-4 w-20 h-20 rounded-full opacity-20 group-hover:opacity-30 transition-opacity"
                      style={{ background: `radial-gradient(circle, ${genre.color}, transparent)` }}
                    />
                    <span className="relative z-10 font-bold text-lg text-white tracking-tight drop-shadow-lg">{genre.name}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === "buscar" && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 border-b border-white/5 pb-4">
                {tracks.length > 0 && (
                  <button
                    onClick={() => { setQuery(""); setTracks([]); setArtistCardData(null); }}
                    className="text-text-secondary hover:text-white transition cursor-pointer flex-shrink-0"
                    title="Volver"
                  >
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                )}
                <h2 className="text-2xl font-bold tracking-tight flex-1">{tracks.length > 0 ? "Resultados de Búsqueda" : "Buscar"}</h2>
              ...
              {tracks.length > 0 ? (
                searchViewMode === "grid" ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 animate-fade-in">
                    {tracks.map((track) => (
                      ...
                      >
                        <div className="relative aspect-video rounded-xl overflow-hidden mb-2 bg-black/40 shadow-sm">
                        ...
                        <h3 className="font-semibold text-sm truncate text-text-primary">{track.title}</h3>
                        <p className="text-xs text-text-secondary truncate mt-0.5">{track.artist}</p>
                      </div>
                    ))}
                  </div>
                ...
                query.trim() === "" && (searchHistory.length > 0 || favorites.length > 0) ? (
                  <div className="animate-fade-in space-y-8">
                    {searchHistory.length > 0 && (
                      <div>
                        <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-4">Últimas búsquedas</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                          {searchHistory.map((item, idx) => {
                            const thumbUrl = searchThumbnails[item];
                            const match = thumbUrl ? (
                              favorites.find(t => t.thumbnail === thumbUrl) ||
                              downloadedMetadata.find(t => t.thumbnail === thumbUrl) ||
                              tracks.find(t => t.thumbnail === thumbUrl)
                            ) : null;
                            return (
                            <button
                              key={idx}
                              onClick={() => performSearchWithQuery(item)}
                              className="group flex flex-col cursor-pointer"
                            >
                              <div className="relative aspect-square rounded-xl overflow-hidden mb-2 bg-black/40 shadow-sm">
                                {thumbUrl ? (
                                  <SafeImage trackId={match?.id} src={thumbUrl} alt={item} className="w-full h-full object-cover group-hover:scale-105 transition" />
                                ) : (
                                  <div
                                    className="w-full h-full flex items-center justify-center"
                                    style={{ background: `hsl(${item.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % 360}, 30%, 20%)` }}
                                  >
                                    <Search className="w-8 h-8 text-white/20" />
                                  </div>
                                )}
                                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition flex items-center justify-center">
                                  <Search className="w-8 h-8 text-white opacity-0 group-hover:opacity-100 transition" />
                                </div>
                              </div>
                              <p className="font-semibold text-sm text-text-primary truncate text-center leading-tight">{item}</p>
                            </button>
                            );
                          })}
                        </div>
                    ...
                    {favorites.length > 0 && (
                      <div>
                        <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-4">Recomendaciones</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          ...
          {activeTab === "artist" && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center mb-4">
                <button
                  onClick={() => { setQuery(""); setTracks([]); navigateTo("buscar"); }}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                                    <button onClick={(e) => { e.stopPropagation(); if (currentTrack?.id === track.id) { togglePlay(); } else { playTrack(track, mappedTracks); } }} className="w-8 h-8 bg-brand-primary text-bg-dark rounded-full flex items-center justify-center">
                                      {currentTrack?.id === track.id && isPlaying ? (
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); openContextMenu(e, track.id); }} className="p-1.5 text-text-secondary hover:text-brand-primary">
                                      <MoreVertical className="w-4 h-4" />
                                    </button>
                                  </div>
                                </div>
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <button 
                    onClick={() => setLibTab("home")}
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition ${
                      libTab === "home" ? "bg-brand-primary text-bg-dark" : "bg-white/5 hover:bg-white/10"
                    }`}
                  >
                    Inicio
                  </button>
              ...
              {libTab === "home" && (
                <div className="space-y-6 animate-fade-in">
                  <h3 className="text-base font-bold text-text-primary border-l-4 border-brand-primary pl-2">
                    Tus Playlists
                  </h3>
                  {playlists.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-4">
                      <Music className="w-12 h-12 text-text-secondary/40" />
                      <p className="text-sm text-text-secondary">Aún no tienes listas de reproducción.</p>
                      <button
                        onClick={() => setShowCreatePlaylistModal(true)}
                        className="px-4 py-2 bg-brand-primary text-bg-dark rounded-xl text-xs font-bold hover:scale-105 transition"
                      >
                        Crear primera lista
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 animate-fade-in">
                      {playlists.map((playlist) => {
                        const thumb = getPlaylistCover(playlist);
                        return (
                          <div
                            key={playlist.id}
                            onClick={() => { setSelectedPlaylistId(playlist.id); navigateTo("playlists"); }}
                            onContextMenu={(e) => openPlaylistContextMenu(e, playlist)}
                            className="flex flex-col group cursor-pointer"
                          >
                            <div className="relative w-full aspect-square rounded-xl overflow-hidden bg-surface-dark/40 mb-2">
                              {thumb ? (
                                <SafeImage src={thumb} alt={playlist.name} className="w-full h-full object-cover" />
                              ) : (
                                <div className="w-full h-full flex items-center justify-center">
                                  <Music className="w-8 h-8 text-text-secondary/40" />
                                </div>
                              )}
                              ...
                              <button
                                ...
                                className="absolute bottom-2 right-2 w-10 h-10 rounded-full bg-brand-primary text-bg-dark shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-200 hover:scale-105"
                              >
                                <Play className="w-5 h-5 ml-0.5" />
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); openPlaylistContextMenu(e, playlist); }}
                                className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                              >
                                <MoreVertical className="w-3.5 h-3.5" />
                              </button>
                            </div>
                            <p className="text-xs font-semibold text-text-primary truncate">{playlist.name}</p>
                            <p className="text-[11px] text-text-secondary/70 truncate">{playlist.tracks.length} canciones</p>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {downloadedMetadata.length > 0 && (
                      <button
                        ...
                        className="p-2 text-text-secondary hover:text-white rounded-lg hover:bg-white/10 transition relative group"
                        title="Abrir carpeta de descargas"
                      >
                        <FolderOpen className="w-4 h-4" />
                        <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2 py-0.5 bg-surface-dark text-[10px] text-text-primary rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                          Abrir carpeta
                        </span>
                      </button>
                    )}
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div className="w-full lg:w-auto flex-shrink-0 lg:sticky lg:top-0 flex flex-col items-center gap-3">
                          ...
                          <div className="w-44 h-44 lg:w-56 lg:h-56 rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-surface-dark flex items-center justify-center relative group">
                            {getPlaylistCover(playlist) ? (
                              <SafeImage src={getPlaylistCover(playlist)} alt={playlist.name} className="w-full h-full object-cover" />
                            ) : (
                            ...
                            <button
                              onClick={() => {
                                ...
                                input.onchange = async (e) => {
                                  ...
                                  reader.onload = (ev) => {
                                    ...
                                    const stored = JSON.parse(localStorage.getItem("capi_playlist_covers") || "{}");
                                    stored[playlist.id] = dataUrl;
                                    localStorage.setItem("capi_playlist_covers", JSON.stringify(stored));
                                    setPlaylists(prev => prev.map(p =>
                                      p.id === playlist.id ? { ...p, _coverOverride: dataUrl } : p
                                    ));
                                  };
                              ...
                              className="absolute bottom-2 right-2 w-7 h-7 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                          ...
                          <div className="w-full min-w-0 text-center">
                            ...
                            <h3 className="text-2xl font-extrabold tracking-tight mb-0.5 text-white break-words">{playlist.name}</h3>
                            <p className="text-xs text-text-secondary">{playlist.tracks.length} canciones</p>
                              ...
                              {playlist.tracks.length > 0 && (
                                <>
                                  ...
                                  <button
                                    ...
                                    className="w-14 h-14 rounded-full bg-brand-primary text-bg-dark flex items-center justify-center hover:scale-105 transition cursor-pointer shadow-lg"
                                    title="Reproducir"
                                  >
...
<SafeImage trackId={track.id} src={track.thumbnail} className="w-11 h-11 rounded-lg object-cover shadow-md flex-shrink-0" alt={track.title} />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 animate-fade-in">
                  {/* Create Playlist dashed square */}
                  <div
                    onClick={() => setShowCreatePlaylistModal(true)}
                    className="flex flex-col cursor-pointer group"
                  >
                    <div className="w-full aspect-square rounded-xl border-2 border-dashed border-white/15 hover:border-brand-primary/50 transition-colors flex items-center justify-center bg-surface-dark/10 hover:bg-surface-dark/20 mb-2">
                      <Plus className="w-10 h-10 text-text-secondary/40 group-hover:text-brand-primary group-hover:scale-110 transition-all duration-200" />
                    </div>
                    <p className="text-xs font-semibold text-text-secondary group-hover:text-text-primary truncate text-center">Crear Lista</p>
                  </div>
                  ...
                  {playlists.map((playlist) => (
                    <div
                      ...
                      className="flex flex-col group cursor-pointer"
                    >
                      <div className="relative w-full aspect-square rounded-xl overflow-hidden bg-surface-dark/40 mb-2 group">
                        {getPlaylistCover(playlist) ? (
                          <SafeImage src={getPlaylistCover(playlist)} alt={playlist.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center">
          ...
          {activeTab === "album_view" && currentAlbum && (
            <div className="flex flex-col lg:flex-row gap-8 items-start animate-fade-in">
              ...
              <div className="w-full lg:w-auto flex-shrink-0 lg:sticky lg:top-0 flex flex-col items-center gap-3">
                ...
                <div className="w-44 h-44 lg:w-56 lg:h-56 rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-surface-dark flex items-center justify-center">
                  <SafeImage src={currentAlbum.thumbnail} alt={currentAlbum.title} className="w-full h-full object-cover" />
                </div>
                <div className="w-full min-w-0 text-center">
                  <p className="text-xs text-brand-primary uppercase font-bold tracking-wider mb-1">
                    {currentAlbum.type === "album" ? "Álbum" : "Lista de Reproducción"}
                  </p>
                  <h2 className="text-2xl font-extrabold tracking-tight mb-0.5 text-white break-words">{currentAlbum.title}</h2>
                  {currentAlbum.artist && (
                    <p className="text-xs text-text-secondary mt-0.5">De <span className="text-white font-semibold">{currentAlbum.artist}</span></p>
                  )}
          ...
          {activeTab === "stats" && (
            <div className="space-y-6 w-full animate-fade-in text-left">
              ...
              {showShareModal && (
                <>
                  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowShareModal(false)}>
                    <div
                      className="glass rounded-3xl p-8 max-w-sm w-full border border-white/10 shadow-2xl shadow-brand-primary/10 text-center"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {/* Logo */}
                      <div className="w-16 h-16 rounded-2xl overflow-hidden mx-auto mb-3 shadow-lg bg-surface-dark border border-white/10">
                        <img src={Logo} alt="Capi" className="w-full h-full object-cover" />
                      </div>

                      {/* Title */}
                      <h3 className="text-xl font-extrabold text-text-primary mb-1">Mi resumen en Capi</h3>
                      <p className="text-xs text-text-secondary/70 mb-5 leading-relaxed">
                        {Object.keys(listeningStats.trackPlays).length > 0
                          ? `${(listeningStats.totalMinutes / 60).toFixed(0)} horas escuchando música. ${Object.keys(listeningStats.trackPlays).length} canciones después, sigo descubriendo sonidos nuevos.`
                          : "Cada escucha cuenta una historia. Descubre la tuya con Capi."}
                      </p>

                      {/* Stats */}
                      <div className="space-y-2.5 mb-5">
                        <div className="bg-gradient-to-r from-brand-primary/15 to-transparent rounded-xl p-3 flex items-center justify-between border border-brand-primary/10">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-xs text-text-secondary">Horas escuchadas</span>
                          </div>
                          <span className="text-sm font-bold text-brand-primary">{(listeningStats.totalMinutes / 60).toFixed(1)} h</span>
                        </div>
                        <div className="bg-white/5 rounded-xl p-3 flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-text-primary">
                          ...
                          <span className="text-sm font-bold text-text-primary">{Object.keys(listeningStats.trackPlays).length}</span>
                        </div>
                        <div className="bg-white/5 rounded-xl p-3 flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-text-primary">
                            ...
                            <span className="text-xs text-text-secondary">Artista favorito</span>
                          </div>
                          <span className="text-sm font-bold text-text-primary truncate max-w-[160px]">{getTopArtists().length > 0 ? getTopArtists()[0].name : "Ninguno"}</span>
                        </div>
                      </div>

                      {/* Invite message */}
                      <p className="text-[11px] text-text-secondary/50 mb-5 leading-relaxed italic">
                        "La música es el lenguaje universal. Descarga Capi y descubre la tuya."
                      </p>
                      ...
                      <button
                        onClick={() => {
                          ...
                          ctx.beginPath();
                          ctx.arc(canvas.width - 60, 60, 140, 0, Math.PI * 2);
                          ctx.fill();
                          ctx.beginPath();
                          ctx.arc(50, canvas.height - 50, 100, 0, Math.PI * 2);
                          ctx.fill();
                          ...
                          // Data for later
                          const hours = (listeningStats.totalMinutes / 60).toFixed(0);
                          const totalHours = (listeningStats.totalMinutes / 60).toFixed(1);
                          const songs = Object.keys(listeningStats.trackPlays).length;
                          const artist = getTopArtists().length > 0 ? getTopArtists()[0].name : "—";

                          const drawContent = () => {
                            // Logo
                            ctx.save();
                            ctx.beginPath();
                            ctx.roundRect(canvas.width / 2 - 36, 50, 72, 72, 16);
                            ctx.clip();
                            const logoImg = new Image();
                            logoImg.onload = () => {
                              ctx.drawImage(logoImg, canvas.width / 2 - 36, 50, 72, 72);
                              ctx.restore();
                              ctx.strokeStyle = "rgba(255,255,255,0.08)";
                              ctx.lineWidth = 1;
                              ctx.beginPath();
                              ...
                              ctx.fillText("Mi resumen en Capi", canvas.width / 2, 170);

                              // Subtitle
                              const msg = songs > 0
                                ? `${hours} horas escuchando música. ${songs} canciones después, sigo descubriendo sonidos nuevos.`
                                : "Cada escucha cuenta una historia. Descubre la tuya con Capi.";
                              ctx.fillStyle = "rgba(255,255,255,0.5)";
                              ctx.font = "13px system-ui, -apple-system, sans-serif";
                              wrapText(ctx, msg, canvas.width / 2, 200, canvas.width - 80, 20, "center");
                              ...
                              const gap = 8;
                              const stats = [
                                { label: "Horas escuchadas", value: totalHours, accent: true },
                                { label: "Canciones", value: `${songs}`, accent: false },
                                { label: "Artista favorito", value: artist, accent: false },
                              ];

                              stats.forEach((s, i) => {
                                const y = statY + i * (statH + gap);
                                if (s.accent) {
                                  ctx.fillStyle = "rgba(124, 58, 237, 0.12)";
                                  ctx.beginPath();
                                  ctx.roundRect(40, y, canvas.width - 80, statH, 12);
                                  ctx.fill();
                                ...
                                ctx.font = "13px system-ui, -apple-system, sans-serif";
                                ctx.textAlign = "left";
                                ctx.fillText(s.label, 60, y + 34);

                                ctx.fillStyle = s.accent ? "#d0bcff" : "#ffffff";
                                ctx.font = "bold 14px system-ui, -apple-system, sans-serif";
                                ctx.textAlign = "right";
                                ctx.fillText(s.value, canvas.width - 60, y + 34);
                              });
                              ...
                              ctx.textAlign = "center";
                              ctx.fillText("\"La música es el lenguaje universal. Descarga Capi y descubre la tuya.\"", canvas.width / 2, 530);

                              // Bottom brand
                              ctx.fillStyle = "rgba(255,255,255,0.12)";
                              ctx.font = "bold 14px system-ui, -apple-system, sans-serif";
                              ctx.textAlign = "center";
                              ctx.fillText("Capi Music Player", canvas.width / 2, 620);
                              ctx.fillStyle = "rgba(255,255,255,0.08)";
                              ctx.font = "11px system-ui, -apple-system, sans-serif";
                              ctx.fillText("capi.app", canvas.width / 2, 640);
                              ...
                              canvas.toBlob((blob) => {
                                ...
                                const a = document.createElement("a");
                                a.href = url;
                                a.download = "mi-resumen-capi.png";
                                a.click();
                                URL.revokeObjectURL(url);
                              }, "image/png");
                            };
                        ...
                        className="w-full py-3 bg-brand-primary text-bg-dark rounded-xl font-bold text-sm hover:scale-105 active:scale-95 transition flex items-center justify-center gap-2"
                      >
                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              {/* Line Chart: Daily usage over 7 days */}
              <div className="bg-surface-dark/40 border border-white/5 rounded-2xl p-6">
                <h3 className="text-xs uppercase font-bold tracking-wider text-text-secondary mb-4 flex items-center gap-2">
                  <BarChart3 className="w-4 h-4 text-brand-primary" /> Uso diario (7 días)
                </h3>
                {(() => {
                  ...
                  const smoothPath = (closed = false) => {
                    const pts = weekData.map((day, i) => ({
                      x: padX + (i / (weekData.length - 1 || 1)) * chartW,
                      y: padY + chartH - (day.minutes / maxMin) * chartH,
                    }));
                    ...
                    if (closed) {
                      d += ` L ${pts[pts.length - 1].x},${padY + chartH} L ${pts[0].x},${padY + chartH} Z`;
                    }
                    return d;
                  };
                    ...
                    <div className="w-full overflow-x-auto">
                      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
                        ...
                        {[0, 0.25, 0.5, 0.75, 1].map((frac) => {
                          ...
                          return (
                            <g key={frac}>
                              <line x1={padX} y1={y} x2={w - padX} y2={y} stroke="white" strokeOpacity={0.06} strokeWidth="1" />
                              <text x={padX - 6} y={y + 3} textAnchor="end" fill="white" fillOpacity="0.3" fontSize="9">{Math.round(maxMin * frac)}</text>
                            </g>
                          );
                        })}
                        <defs>
                        ...
                        {/* Area fill */}
                        <path d={smoothPath(true)} fill="url(#usageGradient)" opacity={0.3} />
                        {/* Line */}
                        <path d={smoothPath(false)} fill="none" stroke="var(--brand-primary)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                        {/* Labels */}
                        {weekData.map((day, i) => {
                          const x = padX + (i / (weekData.length - 1 || 1)) * chartW;
                          return (
                            <text key={i} x={x} y={padY + chartH + 14} textAnchor="middle" fill="white" fillOpacity="0.4" fontSize="9">{day.label}</text>
                          );
                        })}
                      </svg>
                    </div>
          ...
          {activeTab === "settings" && (
            ...
            <div className="animate-fade-in text-left h-full flex flex-col">
              ...
              <div className="flex-1 grid grid-cols-[200px_1fr] gap-6 overflow-hidden min-h-0">
                ...
                <div ref={settingsCardRef} className="bg-surface-dark/40 border border-white/5 rounded-2xl p-6 space-y-6 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  {/* === APARIENCIA === */}
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            {false && theme === "light-mode" && "Light Mode"}
                            ...
                            {theme === "cherry-blossom" && "Cherry Blossom"}
                            {theme === "nordic" && "Nordic"}
                          ...
                          {showThemeDropdown && (
                            <>
                              ...
                              <div
                                 className="absolute -top-[78px] right-[calc(100%+12px)] z-40 glass rounded-2xl p-1.5 shadow-2xl border border-white/10 space-y-0.5"
                                style={{ width: 200 }}
                              >
                                {[
                                ...
                                ].filter(t => t.id !== "light-mode").map((t) => (
                                  <button
                                    key={t.id}
                                    onClick={() => {
                                      setTheme(t.id);
                      ...
                      <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        ...
                        <div className="relative">
                          ...
                          {showAccentDropdown && (
                            <>
                              <div className="fixed inset-0 z-30" onClick={() => { setShowAccentDropdown(false); setAccentSearchQuery(""); }} />
                              <div
                                 className="absolute -top-[164px] right-[calc(100%+12px)] z-40 glass rounded-2xl p-2 shadow-2xl border border-white/10 flex flex-col"
                                style={{ width: 220 }}
                              >
                                <div className="p-1">
                                  <input 
                      ...
                      <div className="border-b border-white/5 pb-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <h3 className="font-semibold text-sm text-text-primary flex items-center gap-2">
                              <Music className="w-4 h-4 text-brand-primary" /> Modo píldora
                            </h3>
                            <p className="text-xs text-text-secondary mt-1">Convierte la barra inferior de reproducción en una píldora flotante con bordes redondeados.</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <label className="relative inline-flex items-center cursor-pointer">
                              <input 
                                ...
                                onChange={(e) => {
                                  ...
                                  setPillPlayer(val);
                                  localStorage.setItem("capi_pill_player", String(val));
                                }}
                                className="sr-only peer" 
                              />
                              <div className="relative w-11 h-6 bg-neutral-300 dark:bg-white/20 rounded-full peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300/50 after:border after:rounded-full after:h-5 after:w-5 after:shadow-md after:transition-all peer-checked:bg-brand-primary"></div>
                            </label>
                            <button
                              onClick={() => setShowPillRadius(!showPillRadius)}
                              className="p-1.5 text-text-secondary hover:text-text-primary transition"
                            >
                              <ChevronLeft className={`w-4 h-4 transition-transform duration-200 ${showPillRadius ? "-rotate-90" : "rotate-180"}`} />
                            </button>
                          </div>
                        </div>
                        {showPillRadius && (
                          <div className="mt-4 animate-fade-in">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-xs text-text-secondary">Redondeo</span>
                              <span className="text-xs font-semibold text-brand-primary tabular-nums">{pillRadius}px</span>
                            </div>
                            <input
                              type="range" min="0" max="64" step="1" value={pillRadius}
                              onChange={(e) => {
                                const val = parseInt(e.target.value, 10);
                                setPillRadius(val);
                                localStorage.setItem("capi_pill_radius", String(val));
                              }}
                              className="w-full accent-brand-primary h-1.5 rounded-full bg-white/10 appearance-none cursor-pointer"
                            />
                      ...
                      <div>
                        ...
                        <div className="space-y-2.5">
                          {[
                            ...
                            { keys: ["0-9"], desc: "Saltar a % (reproductor expandido)" },
                          ].map((shortcut, i) => (
                ...
                {newReleases.length > 0 ? (
                  newReleases.map((release) => (
                    <div 
                      ...
                      className="flex flex-col group cursor-pointer"
                    >
                      <div className="relative aspect-square w-full rounded-xl overflow-hidden mb-2 bg-black/20">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                          <button className="p-3 bg-brand-primary rounded-full hover:scale-105 transition shadow-lg">
                            <Play className="w-6 h-6 text-black fill-current" />
                          </button>
                        </div>
                      </div>
                      <div className="min-w-0 px-1">
                        <p className="font-semibold text-sm truncate text-white">{release.title}</p>
                        <p className="text-xs text-text-secondary truncate">{release.artist}{release.album ? ` • ${release.album}` : ""}</p>
                      </div>
                    </div>
                  ))
                ) : (
        ...
        <footer 
          ...
          style={{
            ...(extractedColors ? { background: `linear-gradient(to right, ${extractedColors.primary}15, ${extractedColors.secondary}08)`, borderTopColor: `${extractedColors.primary}22` } : {}),
            ...(pillPlayer ? { borderRadius: `${pillRadius}px` } : {}),
          }}
          className={`h-24 bg-surface-dark/80 backdrop-blur-md border-t border-white/5 flex items-center justify-between px-8 absolute bottom-0 left-0 right-0 z-20 shadow-2xl cursor-pointer ${
            pillPlayer ? "mx-4 mb-3 border border-white/10 shadow-2xl shadow-black/30 w-[calc(100%-2rem)]" : ""
          }`}
        >
          {/* Left: Info */}
          <div 
          ...
          <div className="w-1/3 flex flex-col items-center gap-2" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-5">
              ...
              >
                {buffering ? <Loader2 className="w-5 h-5 animate-spin" /> : isPlaying ? <Pause className="w-5 h-5 fill-current" /> : <Play className="w-5 h-5 fill-current" />}
              </button>
              <button 
            ...
            <div className="w-full flex items-center gap-3">
              <span id="player-time-min" className="text-[10px] text-text-secondary w-8 text-right cursor-pointer hover:text-brand-primary transition" onClick={toggleTimeDisplay}>{showRemaining ? `-${formatTime(Math.max(0, duration - currentTime))}` : formatTime(currentTime)}</span>
              <input 
                id="player-slider-min"
                type="range" min="0" max={duration || 100} onChange={handleSeek} disabled={!streamUrl}
                onClick={(e) => e.stopPropagation()}
          ...
          <div className="w-1/3 flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Cast Button in Mini Player */}
            <button
              onClick={(e) => { e.stopPropagation(); setShowCastModal(true); }}
              className={`p-2 transition relative ${castSession.state === "connected" ? "text-brand-primary cast-active-pulse rounded-full" : "text-text-secondary hover:text-text-primary"}`}
              title={castSession.state === "connected" ? `Transmitiendo en ${castSession.device?.name || "Dispositivo"}` : "Transmitir audio (Cast)"}
            >
              <CastIcon className="w-4 h-4" />
              {castSession.state === "connected" && (
        ...
        {isPlayerExpanded && currentTrack && (
          <div 
            style={extractedColors ? { background: `radial-gradient(circle at center, ${extractedColors.primary}25 0%, #0c0a0f 100%)` } : {}}
            className={`absolute inset-0 bg-bg-dark/95 z-50 flex flex-col p-6 md:p-8 overflow-hidden animate-fade-in backdrop-blur-xl select-none ${isFocusMode ? "justify-between" : ""}`}
          >
            {/* Ambient Blurred Background Art */}
            <div className="absolute inset-0 z-0 overflow-hidden filter blur-3xl opacity-25 pointer-events-none scale-125">
              <SafeImage src={currentTrack.thumbnail} alt="" className="w-full h-full object-cover" />
            </div>

            {/* ═══════════════════════════════════════════════════════════════
                FOCUS MODE VIEW (Full Screen Inmersive Lyrics)
                ═══════════════════════════════════════════════════════════════ */}
            {isFocusMode ? (
              <div className="relative z-10 flex flex-col h-full w-full max-w-5xl mx-auto justify-between">
                {/* Focus Top Bar */}
                <div className="w-full flex items-center justify-between py-2 border-b border-white/5">
                  {/* Exit Focus Button */}
                  <button 
                    onClick={() => setIsFocusMode(false)}
                    className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-white transition flex items-center gap-2 text-xs font-semibold"
                    title="Salir del Modo Focus"
                  >
                    <ChevronLeft className="w-4 h-4 text-brand-primary" />
                    <span className="hidden sm:inline">Salir de Focus</span>
                  </button>

                  {/* Track Info in Focus */}
                  <div className="text-center px-4 max-w-md">
                    <h2 className="text-base md:text-lg font-extrabold text-white tracking-tight truncate">{currentTrack.title}</h2>
                    <p className="text-xs md:text-sm text-brand-primary font-medium truncate">{currentTrack.artist}</p>
                  </div>

                  {/* Focus Header Action Toolbar */}
                  <div className="flex items-center gap-1.5">
                    {/* Glasses Button (Active state) */}
                    <button
                      onClick={() => setIsFocusMode(false)}
                      className="p-2 rounded-xl bg-brand-primary text-bg-dark font-bold shadow-lg shadow-brand-primary/25 hover:scale-105 transition flex items-center gap-1.5 text-xs"
                      title="Desactivar Modo Focus"
                    >
                      <GlassesIcon className="w-4 h-4" />
                      <span className="hidden md:inline font-mono text-[10px]">FOCUS</span>
                    </button>

                    {/* Sync vs Plain Text Switch */}
                    {(parsedLyrics.length > 0 || lyricsText) && (
                      <div className="flex items-center bg-white/10 p-0.5 rounded-xl border border-white/15">
                        <button
                          onClick={() => setLyricsDisplayMode("synced")}
                          className={`px-3 py-1 text-[11px] rounded-lg font-bold transition flex items-center gap-1.5 ${
                            lyricsDisplayMode === "synced" ? "bg-brand-primary text-bg-dark shadow-md" : "text-text-secondary hover:text-white"
                          }`}
                          title="Modo sincronizado con música"
                        >
                          <Mic2 className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Sincronizada</span>
                        </button>
                        <button
                          onClick={() => setLyricsDisplayMode("plain")}
                          className={`px-3 py-1 text-[11px] rounded-lg font-bold transition flex items-center gap-1.5 ${
                            lyricsDisplayMode === "plain" ? "bg-brand-primary text-bg-dark shadow-md" : "text-text-secondary hover:text-white"
                          }`}
                          title="Modo lectura natural de texto"
                        >
                          <FileText className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Texto plano</span>
                        </button>
                      </div>
                    )}
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`p-2 rounded-xl transition text-xs font-bold ${
                          showTranslation ? "bg-brand-primary/20 text-brand-primary border border-brand-primary/30" : "bg-white/5 hover:bg-white/10 text-text-secondary hover:text-white"
                        }`}
                        title={showTranslation ? "Ocultar traducción" : "Traducir letras"}
                      >
                        <Languages className="w-4 h-4" />
                      </button>

                      {showLangSelector && (
                        <div className="absolute right-0 top-full mt-2 bg-surface-dark border border-white/10 rounded-2xl shadow-2xl z-50 w-48 py-2 modal-content">
                          <p className="px-3 py-1 text-[10px] uppercase tracking-wider text-text-secondary font-bold border-b border-white/5 mb-1">
                            Traducir letra a:
                          </p>
                          <div className="max-h-60 overflow-y-auto px-1 space-y-0.5">
                            {TRANSLATION_LANGS.map(lang => (
                              <button
                                ...
                                className="w-full text-left px-3 py-1.5 text-xs rounded-xl hover:bg-brand-primary hover:text-bg-dark text-text-primary transition flex items-center justify-between"
                              >
                                <span>{lang.name}</span>
                    ...
                    {/* Scroll Lock Toggle */}
                    {lyricsDisplayMode === "synced" && isLyricsSynced && (
                      <button
                        onClick={() => {
                          const nextVal = !lyricsScrollLocked;
                          setLyricsScrollLocked(nextVal);
                          if (nextVal) {
                            setTimeout(() => syncLyricsScroll(true), 50);
                          }
                        }}
                        className={`p-2 rounded-xl transition text-xs font-bold ${
                          lyricsScrollLocked ? "bg-brand-primary/15 text-brand-primary" : "bg-white/5 text-text-secondary hover:text-white"
                        }`}
                        title={lyricsScrollLocked ? "Desbloquear scroll libre" : "Bloquear scroll automático"}
                      >
                        {lyricsScrollLocked ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
                      </button>
                    )}

                    {/* Lyrics Provider Menu Button */}
                    <div className="relative">
                      <button 
                        onClick={() => setLyricsMenuOpen(!lyricsMenuOpen)} 
                        className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-text-secondary hover:text-white transition flex items-center gap-1.5 text-xs"
                        title="Proveedor de letras"
                      >
                        <span className="text-[10px] font-mono text-brand-primary hidden sm:inline">{lyricsProvider || "Auto"}</span>
                        <MoreVertical className="w-4 h-4" />
                      </button>

                      {lyricsMenuOpen && (
                        <div ref={lyricsMenuRef} className="absolute right-0 top-full mt-2 bg-surface-dark border border-white/10 rounded-2xl shadow-2xl z-50 w-60 py-2 modal-content">
                          <div className="px-3 py-1.5 text-[10px] uppercase tracking-wider text-text-secondary font-bold flex items-center justify-between border-b border-white/5 mb-1">
                            <span>Proveedor</span>
                            <span className="text-brand-primary font-mono">{lyricsProvider || "Automático"}</span>
                          </div>
                          <div className="space-y-0.5 px-1">
                            {[
                              { key: null, label: "Automático (Recomendado)" },
                              { key: "YouTube Music", label: "YouTube Music" },
                              { key: "YouTube Subtitle", label: "YouTube Subtitle" },
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (currentTrack) {
                                    setLyricsProvider(opt.key);
                                    fetchLyrics(currentTrack, opt.key ?? undefined);
                                    setLyricsMenuOpen(false);
                                  }
                                }}
                                className={`w-full text-left px-3 py-1.5 text-xs rounded-xl transition flex items-center justify-between ${
                                  (opt.key === null && !lyricsProvider) || opt.key === lyricsProvider
                          ...
                          <button 
                            onClick={() => { setShowLangSelector(true); setLyricsMenuOpen(false); }}
                            className="w-full text-left px-3 py-2 text-xs hover:bg-brand-primary hover:text-bg-dark rounded-xl transition flex items-center gap-2"
                          >
                            <Languages className="w-3.5 h-3.5" /> Traducir letras a otro idioma...
                          </button>
                          <button 
                            onClick={copyLyrics} 
                            className="w-full text-left px-3 py-2 text-xs hover:bg-brand-primary hover:text-bg-dark rounded-xl transition flex items-center gap-2"
                          >
                            <Copy className="w-3.5 h-3.5" /> Copiar letra completa
                          </button>
                          <button 
                            onClick={() => { 
                              if (currentTrack) fetchLyrics(currentTrack, lyricsProvider ?? undefined, true); 
                              setLyricsMenuOpen(false); 
                            }} 
                            className="w-full text-left px-3 py-2 text-xs hover:bg-brand-primary hover:text-bg-dark rounded-xl transition flex items-center gap-2"
                          >
                            <RefreshCw className="w-3.5 h-3.5" /> Recargar letras (Limpiar caché)
                          </button>
                        </div>
                      )}
                    </div>
                    ...
                    <button 
                      ...
                      className="p-2 bg-white/5 hover:bg-white/10 text-white rounded-full transition"
                ...
                {/* Focus Lyrics Content (Edge to Edge, Clean Dynamic Backdrop) */}
                <div 
                  ref={lyricsContainerRef} 
                  className={`flex-1 w-full max-w-4xl mx-auto overflow-y-auto px-6 py-16 space-y-8 text-center select-none lyrics-mask-fade lyrics-font ${
                    lyricsScrollLocked && lyricsDisplayMode === "synced" && isLyricsSynced ? "overflow-hidden" : "overflow-y-auto"
                  }`}
                >
                  {translating && (
                    <div className="flex items-center justify-center gap-2 py-4 text-brand-primary animate-pulse">
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span className="text-sm font-semibold">Traduciendo letras...</span>
                    </div>
                  )}

                  {lyricsLoadingProvider && (
                    <div className="flex items-center justify-center gap-2 py-4 text-brand-primary animate-pulse">
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span className="text-sm font-semibold">Cargando desde {lyricsLoadingProvider}...</span>
                    </div>
                  )}
                  ...
                  {(() => {
                    ...
                    if (displayLyrics.length > 0) {
                      if (lyricsDisplayMode === "plain" || !isLyricsSynced) {
                        // Natural Text Mode
                        return (
                          <div className="max-w-2xl mx-auto space-y-6 text-left md:text-center py-6 animate-fade-in">
                            {displayLyrics.map((line, idx) => (
                              <p 
                                key={idx} 
                                className="text-xl md:text-2xl lg:text-3xl text-white/85 font-medium leading-relaxed tracking-wide"
                              >
                                {line.text}
                              </p>
                            ))}
                      ...
                      // Synchronized Mode (Apple Music / Spotify Style Focus)
                      return displayLyrics.map((line, idx) => {
                        ...
                        return (
                          <p
                            ...
                            className={`text-2xl md:text-4xl lg:text-5xl leading-tight tracking-tight cursor-pointer ${
                      ...
                      return (
                        <div className="py-24 text-center space-y-3">
                          <p className="text-lg md:text-xl text-white/70 font-medium">{lyricsText}</p>
                          {currentTrack && (
                            <button
                              onClick={() => fetchLyrics(currentTrack, undefined, true)}
                              className="px-4 py-2 bg-brand-primary text-bg-dark rounded-xl font-bold text-xs hover:scale-105 transition"
                            >
                              Buscar en otros proveedores
                            </button>
                          )}
                        </div>
                      );
                ...
                {/* Focus Mode Edge-to-Edge Centered Controls Bar */}
                <div className="w-full max-w-2xl mx-auto flex flex-col gap-3 px-4 pt-2 pb-3 focus-controls-bar">
                  {/* Time Slider */}
                  <div className="w-full flex items-center gap-3">
                    <span id="player-time-exp" className="text-xs font-mono text-text-secondary w-10 text-right cursor-pointer hover:text-brand-primary transition" onClick={toggleTimeDisplay}>
                      {showRemaining ? `-${formatTime(Math.max(0, duration - currentTime))}` : formatTime(currentTime)}
                    </span>
                    <input 
                      id="player-slider-exp" 
                      type="range" 
                      min="0" 
                      max={duration || 100} 
                      onChange={handleSeek}
                      className="flex-1 accent-brand-primary h-1.5 bg-white/10 rounded-full cursor-pointer hover:h-2 transition-all" 
                    />
                    <span className="text-xs font-mono text-text-secondary w-10 text-left">{formatTime(duration)}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between w-full pt-1">
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className={`p-2 rounded-xl transition ${repeatMode !== "none" ? "text-brand-primary bg-brand-primary/10" : "text-text-secondary hover:text-white"}`}
                        title={repeatMode === "none" ? "Repetición: Desactivada" : repeatMode === "one" ? "Repetición: Una canción" : "Repetición: Todo"}
                      >
                      ...
                      <button 
                        ...
                        className={`p-2 rounded-xl transition ${isShuffle ? "text-brand-primary bg-brand-primary/10" : "text-text-secondary hover:text-white"}`}
                        title="Aleatorio"
                      >
                    ...
                    <div className="flex items-center gap-5">
                      <button onClick={playPrev} className="p-2 text-text-secondary hover:text-white transition hover:scale-110 active:scale-95" title="Anterior">
                        <SkipBack className="w-6 h-6" />
                      </button>
                      <button 
                        onClick={togglePlay} 
                        className="w-14 h-14 bg-brand-primary text-bg-dark rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition shadow-2xl shadow-brand-primary/30"
                        title={isPlaying ? "Pausar" : "Reproducir"}
                      >
                        {isPlaying ? <Pause className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 fill-current translate-x-0.5" />}
                      </button>
                      <button onClick={playNext} className="p-2 text-text-secondary hover:text-white transition hover:scale-110 active:scale-95" title="Siguiente">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl transition ${castSession.state === "connected" ? "text-brand-primary bg-brand-primary/10 cast-active-pulse" : "text-text-secondary hover:text-white"}`}
                        title="Transmitir audio (Cast)"
                      >
              ...
              /* ═══════════════════════════════════════════════════════════════
                 STANDARD EXPANDED PLAYER VIEW (2 Columns)
                 ═══════════════════════════════════════════════════════════════ */
              <div className="relative z-10 flex flex-col md:flex-row h-full w-full items-center justify-center gap-8 overflow-y-auto md:overflow-y-hidden">
                <button 
                  onClick={() => setIsPlayerExpanded(false)}
                ...
                <div className="w-full md:w-1/2 flex flex-col items-center justify-center max-w-md gap-6 flex-shrink-0">
                  {/* Cover Art */}
                  <div 
                  ...
                  <div className="flex items-center justify-center gap-4 w-full">
                    ...
                    <button 
                      onClick={() => setShowCastModal(true)} 
                      className={`p-2 transition ${castSession.state === "connected" ? "text-brand-primary cast-active-pulse" : "text-text-secondary hover:text-white"}`}
                      title="Transmitir audio (Cast)"
                    >
                      <CastIcon className="w-5 h-5" />
                    </button>
                  ...
                  <div className="w-full flex items-center gap-3 px-4">
                    <span id="player-time-exp" className="text-xs text-text-secondary w-10 text-right cursor-pointer hover:text-brand-primary transition" onClick={toggleTimeDisplay}>{showRemaining ? `-${formatTime(Math.max(0, duration - currentTime))}` : formatTime(currentTime)}</span>
                    <input id="player-slider-exp" type="range" min="0" max={duration || 100} onChange={handleSeek}
                ...
                <div className="w-full md:w-1/2 h-[380px] md:h-[520px] flex flex-col max-w-lg border border-white/5 rounded-3xl bg-surface-dark/30 backdrop-blur-md overflow-hidden flex-shrink-0">
                  {/* Tab bar */}
                    ...
                    {playerTab === "lyrics" && (
                      <div className="flex flex-col h-full relative">
                        <div className="px-4 pt-3 pb-2 flex items-center justify-between border-b border-white/5">
                          <div className="flex items-center gap-2">
                            <span className="text-xs uppercase font-bold tracking-wider text-brand-primary">Letras</span>
                            {showTranslation && (
                          ...
                          <div className="relative flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="p-1.5 bg-white/5 hover:bg-brand-primary hover:text-bg-dark text-text-secondary rounded-lg transition flex items-center gap-1 text-xs font-bold"
                              title="Modo Focus"
                            >
                              <GlassesIcon className="w-4 h-4" />
                              <span className="hidden sm:inline text-[10px]">Focus</span>
                            </button>
                            ...
                            {(parsedLyrics.length > 0 || lyricsText) && (
                              <div className="flex items-center bg-white/10 p-0.5 rounded-lg border border-white/15">
                                <button
                                  onClick={() => setLyricsDisplayMode("synced")}
                                  className={`px-2 py-1 text-[10px] rounded-md font-bold transition flex items-center gap-1 ${
                                    lyricsDisplayMode === "synced" ? "bg-brand-primary text-bg-dark shadow" : "text-text-secondary hover:text-white"
                                  }`}
                                  title="Modo sincronizado"
                                >
                                  <Mic2 className="w-3 h-3" /> Sinc.
                                </button>
                                <button
                                  onClick={() => setLyricsDisplayMode("plain")}
                                  className={`px-2 py-1 text-[10px] rounded-md font-bold transition flex items-center gap-1 ${
                                    lyricsDisplayMode === "plain" ? "bg-brand-primary text-bg-dark shadow" : "text-text-secondary hover:text-white"
                                  }`}
                                  title="Modo texto plano"
                                >
                            ...
                            <div className="relative">
                              <button
                                ...
                                className={`p-1.5 transition text-xs font-bold rounded-lg ${
                                  showTranslation ? "text-brand-primary bg-brand-primary/10" : "text-text-secondary hover:text-brand-primary"
                                }`}
                                title={showTranslation ? "Ver original" : "Traducir letras"}
                              >
                                <Languages className="w-4 h-4" />
                              </button>

                              {showLangSelector && (
                                <div className="absolute right-0 top-full mt-1 bg-surface-dark border border-white/10 rounded-xl shadow-2xl z-50 w-44 py-1 modal-content">
                                  <p className="px-3 py-1 text-[9px] uppercase tracking-wider text-text-secondary font-bold border-b border-white/5 mb-1">
                                    Idioma
                                  </p>
                                  <div className="max-h-48 overflow-y-auto px-1 space-y-0.5">
                                    {TRANSLATION_LANGS.map(lang => (
                                      <button
                                        ...
                                        className="w-full text-left px-2.5 py-1 text-xs rounded-lg hover:bg-brand-primary hover:text-bg-dark text-text-primary transition flex items-center justify-between"
                                      >
                                        <span>{lang.name}</span>
                                        {translationLang === lang.code && <span className="w-1.5 h-1.5 rounded-full bg-brand-primary" />}
                                      </button>
                            ...
                            {lyricsDisplayMode === "synced" && isLyricsSynced && (
                              <button
                                ...
                                className={`p-1.5 transition text-xs font-bold rounded-lg ${
                                  lyricsScrollLocked ? "text-brand-primary bg-brand-primary/10" : "text-text-secondary hover:text-brand-primary"
                                }`}
                            ...
                            {lyricsMenuOpen && (
                              <div ref={lyricsMenuRef} className="absolute right-0 top-full mt-1 bg-surface-dark border border-white/10 rounded-xl shadow-2xl z-50 w-56 py-1 modal-content">
                                <div className="px-3 py-2 text-[10px] uppercase tracking-wider text-text-secondary/60 font-bold flex items-center gap-2 border-b border-white/5 mb-1">
                                  <span className="flex-1">Proveedor</span>
                                  <span className="text-brand-primary text-[10px] font-mono">{lyricsProvider || "auto"}</span>
                                </div>
                                <div className="space-y-0.5 px-1">
                                  {[
                                    { key: null, label: "Automático (más rápido)" },
                                    { key: "YouTube Music", label: "YouTube Music" },
                                    { key: "YouTube Subtitle", label: "YouTube Subtitle" },
                                    { key: "LrcLib", label: "LrcLib" },
                                    { key: "KuGou", label: "KuGou" },
                                    ...
                                    <button
                                      ...
                                      className={`w-full text-left px-3 py-1.5 text-xs rounded-lg transition flex items-center justify-between ${
                                        (opt.key === null && !lyricsProvider) || opt.key === lyricsProvider
                                          ? "bg-brand-primary/10 text-brand-primary font-bold" : "text-text-secondary hover:bg-white/5 hover:text-text-primary"
                                      }`}
                                    >
                                      <span>{opt.label}</span>
                                      {((opt.key === null && !lyricsProvider) || opt.key === lyricsProvider) && (
                                ...
                                <button onClick={() => { if (currentTrack) fetchLyrics(currentTrack, lyricsProvider ?? undefined, true); setLyricsMenuOpen(false); }} className="w-full text-left px-3 py-2 text-xs hover:bg-brand-primary hover:text-bg-dark rounded-lg transition flex items-center gap-2">
                        ...
                        {/* Lyrics Tab Lines Container */}
                        <div 
                          ref={lyricsContainerRef} 
                          className={`flex-1 p-6 space-y-6 text-center text-text-secondary lyrics-mask-fade lyrics-font ${
                            lyricsScrollLocked && lyricsDisplayMode === "synced" && isLyricsSynced ? "overflow-hidden" : "overflow-y-auto"
                          }`}
                        >
                          {lyricsLoadingProvider && (
                            <p className="text-xs text-brand-primary animate-pulse py-4">Buscando letras en {lyricsLoadingProvider}...</p>
                          )}
                              ...
                              if (lyricsDisplayMode === "plain" || !isLyricsSynced) {
                                return (
                                  <div className="space-y-4 text-center py-4 animate-fade-in">
                                    {displayLyrics.map((line, idx) => (
                                      <p key={idx} className="text-base font-semibold text-white/80 leading-relaxed">
                                        {line.text}
                                      </p>
                                    ))}
                                  </div>
                                ...
                                return (
                                  <p
                                    ...
                                    className={`text-lg transition-all duration-300 ${
                                      isActive 
                                        ? "lyric-line-active text-brand-primary" 
                                        : "lyric-line-inactive cursor-pointer"
                                    }`}
                                    onClick={() => {
                                      if (line.time >= 0 && audioRef.current) {
                                        audioRef.current.currentTime = line.time;
        ...
        {/* CAST CONTROL MODAL */}
        {showCastModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[70] modal-overlay" onClick={() => setShowCastModal(false)}>
            <div className="glass p-6 rounded-3xl max-w-sm w-full border border-white/10 shadow-2xl mx-4 modal-content text-white" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-2xl bg-brand-primary/20 text-brand-primary">
                    <CastIcon className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold">Transmitir audio</h3>
                    <p className="text-xs text-text-secondary">Smart TVs y Chromecast</p>
                  </div>
                </div>
                <button onClick={() => setShowCastModal(false)} className="p-1.5 rounded-full hover:bg-white/10 text-text-secondary hover:text-white transition">
                  <X className="w-4 h-4" />
                </button>
              </div>

              {castSession.state === "connected" && castSession.device ? (
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-brand-primary/10 border border-brand-primary/20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white">{castSession.device.name}</p>
                        <p className="text-xs text-brand-primary">
                          {castSession.device.ip ? `IP: ${castSession.device.ip}` : "Transmitiendo stream"}
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      className="px-3 py-1.5 text-xs bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-xl transition font-medium"
                    >
                      Desconectar
                    </button>
                  </div>
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="flex justify-between text-xs text-text-secondary">
                      <span>Volumen de transmisión</span>
                      <span className="font-mono text-brand-primary">{Math.round(castSession.volume * 100)}%</span>
                    </div>
                    <input
                      type="range"
                      min="0"
                      max="1"
                      step="0.05"
                      value={castSession.volume}
                      onChange={e => castManager.setVolume(parseFloat(e.target.value))}
                      className="w-full accent-brand-primary h-1.5 bg-white/10 rounded-full cursor-pointer"
                    />
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Dispositivos en red</p>
                    <button
                      onClick={async () => {
                        await castManager.scanDevices();
                        showToast("Búsqueda de red completada");
                      }}
                      disabled={castSession.isScanning}
                      className="text-[11px] text-brand-primary hover:underline flex items-center gap-1 font-bold"
                    >
                      {castSession.isScanning ? (
                        <>
                          <Loader2 className="w-3 h-3 animate-spin" /> Buscando...
                        </>
                      ) : (
                        <>
                          <RefreshCw className="w-3 h-3" /> Escanear red
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
                    {castManager.getAvailableDevices().length > 0 ? (
                      castManager.getAvailableDevices().map(device => (
                        <button
                          key={device.id}
                          onClick={async () => {
                            await castManager.connect(device, audioRef.current, currentTrack ? { id: currentTrack.id, title: currentTrack.title, artist: currentTrack.artist, url: streamUrl || undefined, thumbnail: currentTrack.thumbnail } : undefined);
                            showToast(`Transmitiendo a ${device.name}`);
                          }}
                          disabled={castSession.state === "connecting"}
                          className="w-full flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 border border-transparent hover:border-white/10 transition group text-left"
                        >
                          <div className="flex items-center gap-3">
                            <div className="p-2 rounded-xl bg-white/5 group-hover:bg-brand-primary/20 group-hover:text-brand-primary transition text-text-secondary">
                              <CastIcon className="w-4 h-4" />
                            </div>
                            <div>
                              <p className="text-sm font-medium text-white group-hover:text-brand-primary transition">{device.name}</p>
                              <p className="text-[11px] text-text-secondary font-mono">
                                {device.ip ? `${device.ip} • ` : ""}{device.model || "Smart TV"}
                              </p>
                            </div>
                          </div>
                          {castSession.state === "connecting" && castSession.device?.id === device.id ? (
                            <Loader2 className="w-4 h-4 text-brand-primary animate-spin" />
                          ) : (
                            <span className="text-xs text-brand-primary opacity-0 group-hover:opacity-100 transition font-medium">Conectar</span>
                          )}
                        </button>
                      ))
                    ) : (
                      <div className="p-4 rounded-2xl bg-white/5 text-center text-xs text-text-secondary space-y-1">
                        <p className="text-white/80 font-medium">No se detectaron dispositivos automáticamente.</p>
                        <p className="text-[11px]">Asegúrate de que tu Smart TV o Chromecast esté en la misma red Wi-Fi.</p>
                      </div>
                    )}
                  </div>

                  {/* Manual IP Direct Connect */}
                  <div className="pt-2 border-t border-white/10">
                    <p className="text-[10px] uppercase font-bold text-text-secondary/70 tracking-wider mb-1.5">Conectar por IP directa</p>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        placeholder="192.168.1.50"
                        value={manualCastIp}
                        onChange={e => setManualCastIp(e.target.value)}
                        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-white placeholder-text-secondary focus:outline-none focus:border-brand-primary font-mono"
                      />
                      <button
                        onClick={async () => {
                          if (!manualCastIp.trim()) {
                            showToast("Ingresa la dirección IP del Smart TV");
                            return;
                          }
                          const dev = castManager.addManualDevice(manualCastIp.trim());
                          await castManager.connect(dev, audioRef.current, currentTrack ? { id: currentTrack.id, title: currentTrack.title, artist: currentTrack.artist, url: streamUrl || undefined, thumbnail: currentTrack.thumbnail } : undefined);
                          showToast(`Transmitiendo a ${dev.name}`);
                        }}
                        className="px-3 py-1.5 bg-brand-primary text-bg-dark rounded-xl font-bold text-xs hover:scale-105 transition"
                      >
                        Conectar
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] modal-overlay" onClick={() => { setContextMenuPlaylist(null); setIsRenamingPlaylist(false); setRenamePlaylistName(""); }}>
          <div className={`glass p-5 rounded-2xl w-full border border-white/10 shadow-2xl mx-4 modal-content ${isRenamingPlaylist ? "max-w-sm" : "max-w-xs"}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-1">
              ...
              {contextMenuPlaylist.tracks.length > 0 && (
                <button 
                  onClick={() => { addTracksToQueue(contextMenuPlaylist.tracks); setContextMenuPlaylist(null); }}
                  className="w-full text-left px-3 py-2.5 text-sm hover:bg-brand-primary hover:text-bg-dark rounded-xl transition flex items-center gap-3"
                >
                  <ListPlus className="w-4 h-4" /> Agregar a la cola
                </button>
              )}
              {contextMenuPlaylist.tracks.length > 0 && (
                <button 
      ...
      {/* ERROR MODAL */}
      {errorModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[90] animate-fade-in" onClick={() => setErrorModal(null)}>
          <div className="glass p-8 rounded-3xl max-w-sm w-full border border-white/10 shadow-2xl mx-4 text-center space-y-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-text-secondary leading-relaxed">{errorModal.message}</p>
            </div>
            <button
              onClick={() => setErrorModal(null)}
              className="px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 text-white text-sm font-medium transition-all duration-200 active:scale-95"
            >
              Cerrar
            </button>
          </div>
      ...
      {/* DIAGNOSTIC MODAL (reemplaza el mensaje genérico para fallos de reproducción) */}
      {diagnosticReport && (
        <DiagnosticModal report={diagnosticReport} onClose={() => setDiagnosticReport(null)} />
      )}
